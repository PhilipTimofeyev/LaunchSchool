...
def winning_arr
  ...
  diag_right = []
  diag_left = []
  (1..BOARD_SIZE).each_slice(board_length) {|n| rows << n.dup; columns_prep << n}
  
  diag_right << (1..BOARD_SIZE).to_a.each_slice(across_slice).map(&:first)
  diag_left << (1..BOARD_SIZE).to_a.rotate(diag_slice_rotation ).each_slice(diag_slice_rotation).map(&:first)[0...-2]
  
  
  board_length.times {columns << columns_prep.map {|arr| arr.shift }}
  
  rows + columns + diag_right + diag_left
end
