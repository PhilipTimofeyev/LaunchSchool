...
def what_board_size
  prompt "Please enter a board size between 3 and 9"
  prompt "Enter '3' for 3x3, '4' for 4x4, or '5' for 5x5, ect."
  board_size = nil
  loop do
    board_size = gets.chomp
    break if integer?(board_size) && board_size.to_i.between?(3, 9)
    prompt "Please enter an integer between 3 and 9."
  end
  board_size.to_i**2
end
...
def ai_find_best_move(brd, win_lines, al_be_hsh)
  best_score = -Float::INFINITY
  best_move = nil
  empty_squares(brd).each do |square|
    brd[square] = COMPUTER_MARKER
    score = minimax(brd, 0, al_be_hsh, false,
                    win_lines)
...
def ai_turn!(brd, win_lines, al_be_hsh)
  if when_to_minimax?(brd)
    prompt "AI is thinking..."
    best_move = ai_find_best_move(brd, win_lines, al_be_hsh)
    sleep(1)
...
def determine_strategic_move(marker, brd, win_lines)
  if ai_strategy(marker, brd, win_lines).any?
    ai_find_squares(marker, brd, win_lines).sample
  end
end

def determine_center_square(brd)
  if brd.size.odd? && brd[(brd.size / 2) + 1] == INITIAL_MARKER
    (brd.size / 2) + 1
  end
end
...
def computer_turn!(brd, win_lines)
  ...
  loop do
    square = determine_strategic_move(COMPUTER_MARKER, brd, win_lines)
    break if !square.nil?
    square = determine_strategic_move(PLAYER_MARKER, brd, win_lines)
    break if !square.nil?
    square = determine_center_square(brd)
    break if !square.nil?
...
def minimax(brd, depth, al_be_hsh, is_maximizing, win_lines)
  if detect_winner(brd, win_lines) || depth >= 5
    return minimax_score(brd, win_lines, depth)
  end

  if is_maximizing
    maximizing(brd, depth, al_be_hsh, win_lines)
  else
    minimizing(brd, depth, al_be_hsh, win_lines)
  end
end

def maximizing(brd, depth, al_be_hsh, win_lines)
  best_score = -Float::INFINITY
  empty_squares(brd).each do |square|
    brd[square] = COMPUTER_MARKER
    score = minimax(brd, depth + 1, al_be_hsh, false, win_lines)
    brd[square] = INITIAL_MARKER
    best_score = [score, best_score].max
    al_be_hsh[:alpha] = [al_be_hsh[:alpha], score].max
    break if al_be_hsh[:beta] <= al_be_hsh[:alpha]
  end
  best_score
end

def minimizing(brd, depth, al_be_hsh, win_lines)
  best_score = Float::INFINITY
  empty_squares(brd).each do |square|
    brd[square] = PLAYER_MARKER
    score = minimax(brd, depth + 1, al_be_hsh, true, win_lines)
    brd[square] = INITIAL_MARKER
    best_score = [score, best_score].min
    al_be_hsh[:beta] = [al_be_hsh[:beta], score].min
    break if al_be_hsh[:beta] <= al_be_hsh[:alpha]
  end
  best_score
end
...
def continue_to_next_round?
  response = nil
  loop do
    prompt "Continue to next round? (yes or no)"
    answer = gets.chomp
...
def play_another_match?
  response = nil
  loop do
    prompt "Play another match? (yes or no)"
    answer = gets.chomp
    if %w(y yes).include?(answer)
      response = true
      break
...
al_be_hsh = { alpha: -Float::INFINITY, beta: Float::INFINITY }
...
loop do
  ...
  loop do
    ...
    players = [Proc.new { player_turn!(board) },
               Proc.new { ai_turn!(board, winning_lines, al_be_hsh) }]
    ...
    break unless continue_to_next_round?
  end
