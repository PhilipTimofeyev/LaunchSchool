...
class Move
  ...
  def scissors_wins?(move1, move2)
    move1.scissors? && (move2.paper? || move2.lizard?)
  end

  def spock_wins?(move1, move2)
    move1.spock? && (move2.scissors? || move2.rock?)
  end

  def lizard_wins?(move1, move2)
    move1.lizard? && (move2.spock? || move2.paper?)
  end

  def >(other_move)
    rock_wins?(self, other_move) ||
      paper_wins?(self, other_move) ||
      scissors_wins?(self, other_move) ||
      spock_wins?(self, other_move) ||
      lizard_wins?(self, other_move)
  end

  def <(other_move)
    rock_wins?(other_move, self) ||
      paper_wins?(other_move, self) ||
      scissors_wins?(other_move, self) ||
      spock_wins?(other_move, self) ||
      lizard_wins?(other_move, self)
  end
