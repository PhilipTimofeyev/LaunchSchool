...
module HardMoveset
  ...
  def minimax_winner
    ...
    when human_marker then return 'Player'
    when marker then return 'Computer'
    else return 'Tie' if board.full?
    end
    nil
  end
...
class TTTGame
  ...
  def ask_which_opponent
    puts "Please select an opponent: #{Computer::OPPONENTS.join(', ')}"
    choice = nil
    loop do
  ...
  def select_opponent
    ...
    when 1 then WallE.new(board, human.marker)
    when 2 then Jarvis.new(board, human.marker)
    when 3 then ExMachina.new(board, human.marker)
    end
  end
