...
module HumanMoveset
  def ask_for_name
    ...
    loop do
      ...
      break if name.length < 12 && !name.empty?
  ...
  def same_markers?(mark)
    mark == human_marker if self.class == HumanOpponent
  end

  def marker_validation(mark)
    mark.length == 2 &&
      !['O', 'J', 'Ʃ'].include?(mark.upcase) &&
      !same_markers?(mark)
  end
  ...
  def choose_marker
    ...
    loop do
      ...
      mark = ' X' if mark.empty?
      break if marker_validation(mark)
      puts "Sorry, that's not a valid choice."
    end
    mark
  end
...
class TTTGame
  ...
  def select_opponent
    ...
    when 1 then HumanOpponent.new(board, human.marker)
  ...
  def ask_to_play_next_round
    ...
    loop do
      puts "\nWould you like to start the next round? (y/n)"
      answer = gets.chomp.downcase
      break if %w(y n).include?(answer)
      puts "Sorry, must be y nor n."
    end
